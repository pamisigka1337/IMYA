...
const DEMO_NOTICE = "Это демонстрационная оплата для дипломного проекта. Реальные деньги не списываются, данные карты не сохраняются";
...
export default function Payment() {
  ...
  const payMutation = useMutation({
    ...
    onSuccess: (_data, paymentMethod) => {
      ...
      const message = paymentMethod === "sbp" ? "Демо-оплата через СБП прошла успешно" : "Демо-оплата прошла успешно";
    ...
    <div className="min-h-screen py-8 md:py-12">
      <div className="mx-auto max-w-4xl px-6">
        ...
        <div className="mb-6 rounded-2xl border border-primary/30 bg-primary/10 p-4 text-sm text-foreground flex gap-3">
          ...
          <div><p className="font-semibold">Демо-оплата для учебного проекта</p><p className="text-muted-foreground">{DEMO_NOTICE}.</p></div>
        </div>
          ...
          <Card className="rounded-2xl border-border/50 h-fit">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2 border-t pt-4 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Количество дней</span><span>{booking.days}</span></div>
          ...
          <Card className="rounded-2xl border-border/50">
            ...
            <CardContent>
              {isPaid || successMessage ? (
                <div className="text-center py-10 space-y-4"><CheckCircle className="h-14 w-14 text-green-600 mx-auto" /><h3 className="text-xl font-semibold">{successMessage || "Оплачено"}</h3><p className="text-muted-foreground">Демо-оплата для учебного проекта завершена.</p><Link href="/account"><Button className="rounded-xl">Вернуться в личный кабинет</Button></Link></div>
              ) : (
                <Tabs defaultValue="card">
                  <TabsList className="grid w-full grid-cols-2 rounded-xl"><TabsTrigger value="card">Банковская карта</TabsTrigger><TabsTrigger value="sbp">СБП</TabsTrigger></TabsList>
                  <TabsContent value="card" className="mt-6 space-y-5">
                    <div className="rounded-2xl bg-gradient-to-br from-zinc-900 to-zinc-700 p-5 text-white shadow-lg"><p className="text-xs opacity-70">DEMO CARD</p><p className="mt-8 text-xl tracking-[0.25em]">{cardForm.number || "0000 0000 0000 0000"}</p><div className="mt-6 flex justify-between text-sm"><span>{cardForm.holder || "CARD HOLDER"}</span><span>{cardForm.expiry || "MM/YY"}</span></div></div>
                    ...
                    <p className="text-xs text-muted-foreground">Демо-оплата для учебного проекта. Поля карты используются только в браузере и не отправляются на сервер.</p>
                    <Button className="w-full rounded-xl" size="lg" disabled={payMutation.isPending} onClick={() => payMutation.mutate("card")}><CreditCard className="mr-2 h-4 w-4" />{payMutation.isPending ? "Обработка демо-оплаты..." : `Оплатить ${total.toLocaleString("ru-RU")} ₽`}</Button>
                  </TabsContent>
                  <TabsContent value="sbp" className="mt-6 space-y-5 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">Демо-режим, реальное списание не выполняется. Демо-оплата для учебного проекта.</p>
